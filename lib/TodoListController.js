...
class TodoListController {
  constructor(todoDB) {
    this.todoDB = todoDB;
    this.todoList = this.todoDB.loadTodoList();
  }
  ...
  toggleComplete(positionInput) {
    ...
    toggle.run({ new: (this.todoList.taskAt(pos - 1).isComplete ? 1 : 0), toToggle: this.todoList.taskAt(pos - 1).id });
  }
  ...
  toggleImportant(positionInput) {
    ...
    toggle.run({ new: (this.todoList.taskAt(pos - 1).isImportant ? 1 : 0), toToggle: this.todoList.taskAt(pos - 1).id });
  }
}
